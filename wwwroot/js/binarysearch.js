// Binary Search Visualizer JavaScript
let searchSteps = [];
let currentStepIndex = 0;
let isAnimating = false;
let isPaused = false;
let animationSpeed = 1000; // Slower for detail
let timer = null;
let startTime = null;
let score = 0;
...
const currentMidEl = document.getElementById('currentMidVal');
...
generateBtn.addEventListener('click', generateSortedArray);
...
async function handleSearchInit(autoPlay) {
    ...
    try {
        const response = await fetch('/BinarySearch/GenerateSteps', {
            method: 'POST',
            headers: {
...
function renderStep(step) {
    ...
    // Sort array to match backend processing (if not already matching)
    // Actually backend returns 'Array' in step, which is sorted.
    ...
    arr.forEach((value, index) => {
        ...
        // Adjust style for label if markers are present
        ...
        if (index === step.mid) {
            ...
            midMarker.className = 'index-marker marker-mid';
            midMarker.textContent = 'MID';
            box.appendChild(midMarker);

            currentMidEl.textContent = value;
        }

        // 3. Is it found?
        if (step.isFound && index === step.mid) {
            box.classList.add('found');
            box.classList.remove('mid'); // Found overrides mid color
        }
    ...
    // Comparisons estimaton
    // We can just track how many mid steps we have passed
    let comps = 0;
    for (let i = 0; i < currentStepIndex && i < searchSteps.length; i++) {
        if (searchSteps[i].mid !== -1) comps++;
    }
...
function handleSearchComplete() {
    ...
    if (isFound) {
        ...
        resultMessageEl.textContent = `Found value ${lastStep.target} at index ${lastStep.mid}.`;

        // Log n score calculation
        const baseScore = 2000;
        const compPenalty = parseInt(comparisonsEl.textContent) * 100;
...
function handleReset() {
    ...
    arrayContainer.innerHTML = '<p style="color: #94a3b8; text-align: center;">Enter array and target to begin. Note: Array will be sorted.</p>';
