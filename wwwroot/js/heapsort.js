// Heap Sort Visualizer JavaScript
let sortingSteps = [];
...
async function handleStartSorting() {
    ...
    try {
        const response = await fetch('/HeapSort/GenerateSteps', {
            method: 'POST',
            headers: {
        ...
        heapSizeEl.textContent = array.length;
...
function renderStep(step) {
    ...
    step.array.forEach((value, index) => {
        ...
        } else if (index === step.compareIndex1 || index === step.compareIndex2) {
            if (step.swapped) {
    ...
    // Update stats
    heapSizeEl.textContent = step.heapSize;

    // Count comparisons and swaps up to current step
    let totalComparisons = 0;
    let totalSwaps = 0;
    for (let i = 0; i <= currentStepIndex; i++) {
        if (sortingSteps[i].compareIndex1 >= 0) {
            totalComparisons++;
        }
        if (sortingSteps[i].swapped) {
            totalSwaps++;
        }
    }
...
function handleSortingComplete() {
    ...
    const finalStep = sortingSteps[sortingSteps.length - 1];
...
function handleHint() {
    ...
    const hints = [
        'Heap Sort first builds a binary heap (max heap for ascending, min heap for descending).',
        'After building the heap, the root element is always the largest (or smallest) unsorted element.',
        'The heapify operation maintains the heap property by comparing parent with children.',
        'Watch the yellow bars - they show which elements are being compared during heapify.',
        'Red bars indicate a swap is happening to maintain the heap property.',
        'Green bars show elements that have been extracted and are in their final sorted position.',
        'Heap Sort has O(n log n) time complexity in all cases - better than Bubble Sort!'
    ];
