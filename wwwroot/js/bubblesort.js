...
async function handleStartSorting() {
    ...
    try {
        const response = await fetch('/BubbleSort/GenerateSteps', {
            method: 'POST',
            headers: {
...
function renderStep(step) {
    ...
    if (step.passNumber > 0) {
        currentPassEl.textContent = step.passNumber;
    }
...
function handleHint() {
    ...
    const hints = [
        ...
        'The algorithm can terminate early if no swaps occur in a pass.',
        'Watch the yellow bars - they show which elements are being compared.',
        'Red bars indicate a swap is happening between two elements.',
