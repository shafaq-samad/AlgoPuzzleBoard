...
async function handleStartSorting() {
    ...
    try {
        const response = await fetch('/MergeSort/GenerateSteps', {
            method: 'POST',
            headers: {
...
function renderStep(step) {
    ...
    // Count comparisons and merges up to current step
    ...
    mergesEl.textContent = totalMerges;
}
...
function handleSortingComplete() {
    ...
    const totalMerges = parseInt(mergesEl.textContent);
    ...
    document.getElementById('resultMerges').textContent = totalMerges;
...
function handleHint() {
    ...
    const hints = [
        ...
        'The array is recursively divided into halves until each subarray has one element.',
        'Single-element subarrays are considered sorted by definition.',
        'Watch the yellow bars - they show which elements are being compared during merge.',
        'Red bars indicate elements being placed in their sorted position.',
        'Green bars show elements that are in their final sorted position.',
        'Merge Sort has O(n log n) time complexity in all cases - very efficient!',
