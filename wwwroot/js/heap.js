// Max Heap Construction Visualizer
let currentArray = [];
let isSolving = false;
...
// Manual Mode State
let manualParentIdx = -1; // Current parent being heapified
...
function setupUIAfterLoad() {
    ...
    updateStatus("Array loaded. Choose Auto or Manual Mode to build the heap.");
}
...
function renderHeap(highlights = [], swaps = []) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (isHighlighted) { stroke = "#fbbf24"; color = "rgba(251, 191, 36, 0.4)"; }
...
function renderArray(highlights = [], swaps = []) {
    arrayContainer.innerHTML = currentArray.map((val, i) => {
        ...
        else if (highlights.includes(i) || (isManualMode && i === manualParentIdx)) { style = "border: 2px solid #fbbf24; background: rgba(251, 191, 36, 0.4); color: #fbbf24; font-weight: bold;"; }
...
async function startBuildHeap() {
    ...
    startTimer();
    const res = await fetch('/Heap/Solve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ array: currentArray }) });
...
function startManualSolve() {
    ...
    updateStatus(`Manual Mode: Start from node ${manualParentIdx}. Click a larger child to swap, or node itself if no violation.`);
}
...
function onNodeClick(index) {
    ...
    let largest = manualParentIdx;
    if (l < n && currentArray[l] > currentArray[largest]) largest = l;
    if (r < n && currentArray[r] > currentArray[largest]) largest = r;

    if (index === manualParentIdx) {
        if (largest === manualParentIdx) {
            currentScore += 10;
            ...
            document.getElementById('currentScoreDisplay').textContent = currentScore;
            updateStatus(`Wrong! Node ${manualParentIdx} (${currentArray[manualParentIdx]}) is smaller than child ${largest} (${currentArray[largest]}). Click the child to swap.`);
        }
    } else if (index === l || index === r) {
        if (index === largest) {
            const temp = currentArray[manualParentIdx];
            currentArray[manualParentIdx] = currentArray[largest];
            currentArray[largest] = temp;
            currentScore += 10;
            document.getElementById('currentScoreDisplay').textContent = currentScore;

            const oldParent = manualParentIdx;
            manualParentIdx = largest;
            ...
            if (nextL >= n) {
                ...
                updateStatus(`Correct swap! Continue heapifying down from index ${manualParentIdx}.`);
            }
        } else {
            currentScore = Math.max(0, currentScore - 5);
            document.getElementById('currentScoreDisplay').textContent = currentScore;
            updateStatus(`Incorrect swap! Child ${index} is not the largest violation.`);
        }
    } else {
...
function renderStep(index) {
    ...
    // For auto-solving, we update score on swaps
    // In manual stepping, we don't strictly enforce score consistency
...
function stepBack() {
    ...
    clearAnimation();
    if (!isPaused) togglePause();
    animationIndex -= 2; // Go back two steps (current and previous)
    renderStep(animationIndex);
    animationIndex++;
}
...
function finishSolve() {
    ...
    document.getElementById('stepControls').style.display = 'none';
    renderHeap(); renderArray();
    updateStatus("Success! Max Heap built.");
}
